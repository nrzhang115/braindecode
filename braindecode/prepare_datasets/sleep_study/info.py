...
HEALTH_DATA_FNS = [DEMOGRAPHIC, DIAGNOSIS, ENCOUNTER, MEASUREMENT, MEDICATION, PROCEDURE_SURG_HX, PROCEDURE, SLEEP_ENC_ID, SLEEP_STUDY_NAME]
INTERVAL = 30 # seconds.
...
def load_health_info(name, convert_datetime=True):
    ...
    path = os.path.join('sleep_study-master2', 'health_data', name)
    df = pd.read_csv(path)
